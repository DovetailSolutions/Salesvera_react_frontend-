...
export default function UserManagement() {
  ...
  useEffect(() => {
    fetchUsers(1, "", isManager ? "all" : roleFilter);
  }, [isManager, user.id]);
  ...
  const handleSearch = (e) => {
    ...
    setSearchTerm(value);
    if (!isManager) {
      // ✅ Pass current roleFilter to avoid stale closure
  ...
  return (
    <div className="px-4 py-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-wrap justify-between items-center gap-4 mb-6">
        <div className="flex flex-wrap items-center gap-4 flex-1 max-w-3xl">
          <input
            ...
            placeholder={
              isManager
                ? "Search not available for managers"
            ...
            className={`px-5 py-2 rounded ${
              isManager
                ...
                : "border-gray-300 focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-transparent"
            } shadow-sm w-full sm:w-auto flex-1 min-w-[200px] custom-border`}
          />
      ...
      <Table
        ...
        onPageChange={(page) =>
          fetchUsers(page, isManager ? "" : searchTerm, isManager ? "all" : roleFilter)
        }
      />
